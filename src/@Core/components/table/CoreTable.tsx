...
const CoreTable: React.FC<CoreTableProps> = ({
	...
	const [checkedItems, setCheckedItems] = useState<any[]>(initChecked ?? [])
					...
					{data.map((item, index) => {
						return (
							...
							>
									...
									<td className="w-4 p-4 px-2 py-6">
										<div className="flex items-center px-2">
											<input
												...
												disabled={item?.disabledCheckbox ?? false}
												type="checkbox"
								...
								{columns.map(column => {
									const { render, key, dataKey } = column
									return (
										key !== 'index' && (
											<td
												key={`row-${key}-${index}`}
												className="px-2 py-6 whitespace-nowrap text-ellipsis"
											>
												<div className="p-6">
													{render
														? render(get(item, dataKey) ?? item)
														: get(item, dataKey) ?? ''}
												</div>
											</td>
										)
